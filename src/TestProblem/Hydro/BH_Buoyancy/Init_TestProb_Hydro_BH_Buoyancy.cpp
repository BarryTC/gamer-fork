...
const  int      NParBoundMin = 10;                       // the minimum number of bound particles considered when determining CM position and velocity
       int      NParBound_Tot;                           // total number of bound particles across all ranks
...
static double   Central_Ini_R_vir;                       // host initial virial radius
static double   Central_Ini_vel_cir;                     // circular velocity at the host virial radius "= SQRT(G*M_vir/Central_Ini_R_vir)"
static double   Most_Bound_Frac_CM_Cal;                  // the fraction of most bound particles used to compute instantaneous CM
       double   Central_CM[3] = { 0.0, 0.0, 0.0 };       // host center of mass coordinates
static double   Central_vel[3] = { 0.0, 0.0, 0.0 };      // host center of mass velocity

#ifdef PARTICLE
void AddNewParticleAttribute_BH_Buoyancy();
void Compute_Den_Max_On_Grid_BH_Buoyancy( double DenMaxOnGridPos[] );
void Output_UserWorkBeforeOutput_BH_Buoyancy();
void Aux_Record_BH_Buoyancy();
#endif
# ifdef GRAVITY
bool Flag_BH_Buoyancy( const int i, const int j, const int k, const int lv, const int PID, const double *Threshold );
void Recompute_Potential_BH_Buoyancy( const int OptExtPot_Input );
void ParEnergy_Calculation_BH_Buoyancy( const bool OPT_Self_Total_Pot );
void ParEnergy_Sorting_BH_Buoyancy( const bool FirstTime_MassRest, long& NParBound_Tot_5percent,
...
// ********************************************************************************************************************************
   ...
   ReadPara->Add( "Most_Bound_Frac_CM_Cal",    &Most_Bound_Frac_CM_Cal,     0.05,          0.00001,          1.0               );
...
// (1-2) set the default values
   for (int i=0; i<3; i++)  Central_CM[i] = 0.5*amr->BoxSize[i];
...
// (3) make a note
   if ( MPI_Rank == 0 )
   {
      ...
      Aux_Message( stdout, "  individual host particle mass                              = %13.7e\n", Particle_Mass );
      Aux_Message( stdout, "  host initial virial radius                                 = %13.7e\n", Central_Ini_R_vir );
      Aux_Message( stdout, "  host initial circular velocity at R_vir                    = %13.7e\n", Central_Ini_vel_cir );
      Aux_Message( stdout, "  fraction of most bound particles for instantaneous CM      = %13.7e\n", Most_Bound_Frac_CM_Cal );
...
//-------------------------------------------------------------------------------------------------------
// Function    :  AddNewParticleAttribute_BH_Buoyancy
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Recompute_Potential_BH_Buoyancy
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Compute_Den_Max_On_Grid_BH_Buoyancy
...
//-------------------------------------------------------------------------------------------------------
// Function    :  ParEnergy_Calculation_BH_Buoyancy
// Description :  Compute individual particle total energy
//
// Note        :  compute "self_potential + kinetic"
...
// 2. potential energy
   if (OPT_Self_Total_Pot)  Recompute_Potential_BH_Buoyancy( EXT_POT_NONE ); // recompute potential on all grids
...
//-------------------------------------------------------------------------------------------------------
// Function    :  ParEnergy_Sorting_BH_Buoyancy
// Description :  Compute individual particle total energy
//
// Note        :  compute "self_potential + kinetic"
//                [Part II] loop thorough all particles to (1) assign unbound particles negative mass and (2) determine the total energy of individual particles
...
// Return      :  None
//-------------------------------------------------------------------------------------------------------
void ParEnergy_Sorting_BH_Buoyancy( const int NParBoundMin, long& NParBound_Tot_5percent,
                        double& CM_Pos_Relative_Error, double& CM_Vel_Relative_Error )
{
...
// see "src/SelfGravity/Poi_GetAverageDensity.cpp"
   if ( MPI_Rank == 0 )
   {
      ...
      for (int r=0; r<MPI_NRank; r++)
      {
         NParBound_Tot += NParBound_AllRank[r];
         Count[r]       = NParBound_AllRank[r];
      }
      for (int r=1; r<MPI_NRank; r++)  Disp[r] = Disp[r-1] + Count[r-1];
   } // if ( MPI_Rank == 0 )

   NParBound_Tot_5percent = MAX(NParBoundMin, round(Most_Bound_Frac_CM_Cal*NParBound_Tot)); // 5% of the total bound particle counts
   double *ParBoundAtt_AllRank[7];                                              // (in order) PosX, PosY, PosZ, VelX, VelY, VelZ, ParEnergy
   for (int v=0; v<7; v++)  ParBoundAtt_AllRank[v] = new double [ NParBound_Tot ];
   long *ParBoundpIdx_AllRank = new long [ NParBound_Tot ];                     // ParTag
   MPI_Bcast( &NParBound_Tot, 1, MPI_INT, 0, MPI_COMM_WORLD );
   MPI_Bcast( &NParBound_Tot_5percent, 1, MPI_LONG, 0, MPI_COMM_WORLD );
   //Aux_Message( stdout, "      NParBound_Tot = %10d\n", NParBound_Tot);
...
// gather additional least unbound particle information if "NParBoundMin > NParBound_Tot"
   if (NParBoundMin > NParBound_Tot)
   {
      Aux_Message( stdout, "      NParBoundMin = %10d, NParBound_Tot = %10d, NParBound_Tot_5percent = %10d\n", NParBoundMin, NParBound_Tot, NParBound_Tot_5percent );
...
//    compute the CM position and velocity from the 5% most bound particles
      if ( MPI_Rank == 0 )
      {
         for (int p=0; p<(NParBoundMin-NParBound_Tot); p++)
         {
...
//    sort particle total energy entries; see "src/Miscellaneous/Mis_Heapsort.cpp" and "/tool/analysis/gamer_compare_data/GAMER_Functions/SortParticle.cpp"
      int *SortByEnergy_IdxTable = new int [ NParBound_Tot ];
      Mis_Heapsort( NParBound_Tot, ParBoundAtt_AllRank[6], SortByEnergy_IdxTable );

      if (NParBoundMin > NParBound_Tot)
      {
         for (int p=0; p<NParBound_Tot; p++)
         {
      ...
      {
         for (int p=0; p<NParBound_Tot_5percent; p++)
         {
            Par_Idx_Now = SortByEnergy_IdxTable[p];
...
//    update the host CM position and velocity
      for (int v=0; v<3; v++)
      {
         CM_Pos[v]  /= (double)NParBound_Tot_5percent;
         CM_Vel[v]  /= (double)NParBound_Tot_5percent;
      }

      CM_Pos_Relative_Error = SQRT(SQR(CM_Pos[0]-Central_CM[0])+SQR(CM_Pos[1]-Central_CM[1])+SQR(CM_Pos[2]-Central_CM[2]))/Central_Ini_R_vir;
      ...
      for (int v=0; v<3; v++)
      {
         Central_CM[v]  = CM_Pos[v];
         Central_vel[v] = CM_Vel[v];
      }

//    record the ParTag list
      for (int p=0; p<NParBound_Tot; p++)  ParBoundpIdx_AllRank[SortByEnergy_IdxTable[p]] = p;
...
// broadcast
   ...
   MPI_Bcast( Central_CM, 3, MPI_DOUBLE, 0, MPI_COMM_WORLD );
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Output_UserWorkBeforeOutput_BH_Buoyancy
...
//
//
// Note        :  1. The function iteratively determine the host CM by selecting a fixed percentage
...
//                   also temporarily disabled, after the host self-potential is recomputed accounting
//                   only for bound members of the host
...
//                4. Recompute the "Central_CM[3]" and "Central_vel[3]" from the 5% most bound particles,
...
//                5. Before exiting the function, we restore particle mass and re-compute the total
...
void Output_UserWorkBeforeOutput_BH_Buoyancy()
{
   ...
   static long NParBound_Tot_5percent = 0;                       // 5% of the total bound particle counts
   int    NIter                       = 0;
...
// adopt the on-grid maximum density position as the initial guess of the host CM
   if ( Step == (real)0 )  Compute_Den_Max_On_Grid_BH_Buoyancy( Central_CM );
// adopt the CM position and velocity inferred from the most bound particles in the previous output
   else if (NParBound_Tot_5percent > NParBoundMin)
   {
      Aux_Message( stdout, "      NParBound_Tot_5percent = %10d, Step = %10d\n", NParBound_Tot_5percent, Step);
      ...
      {
//       skip inactive (with negative mass), and tracer particles
         if ( (amr->Par->Attribute[Idx_ParTag][p] > (real)0.0) && (amr->Par->Attribute[Idx_ParTag][p] <= NParBound_Tot_5percent)
      ...
      if ( MPI_Rank == 0 )
      {
         //Aux_Message( stdout, "      NParBound_Tot_5percent = %10d, ParVel_Sum_AllRank = %10d\n", NParBound_Tot_5percent, ParVel_Sum_AllRank);
         for (int v=0; v<3; v++)
         {
            Central_CM[v]  = CM_Vel_Est_AllRank[v]/(double)ParVel_Sum_AllRank;
            Central_vel[v] = CM_Vel_Est_AllRank[v]/(double)ParVel_Sum_AllRank;
         }
         Aux_Message( stdout, "      After: Central_CM[0] = %14.7e,  Central_CM[1] = %14.7e,  Central_CM[2] = %14.7e\n", Central_CM[0], Central_CM[1], Central_CM[2]);
...
//       sort particle total energy entries
         ...
         for (int v=0; v<3; v++)
         {
            Central_CM[v]  = CM_Pos_Est[v]/(double)NParCMEst;
            Central_vel[v] = CM_Vel_Est[v]/(double)NParCMEst;
         }
         //Aux_Message( stdout, "      (MPI_Rank = %10d) After: Central_CM[0] = %14.7e,  Central_CM[1] = %14.7e,  Central_CM[2] = %14.7e\n", MPI_Rank, Central_CM[0], Central_CM[1], Central_CM[2]);
...
//    [Part II] update individual particle energy entries
      ParEnergy_Calculation_BH_Buoyancy( true );

//    ============================================================================================================
//    [Part III] relabel bound/unbound particles, and perform non-parallel sorting based on the particle total specific energy
      NParBound_Tot = 0;                                // initialize/reset the value
      ParEnergy_Sorting_BH_Buoyancy( NParBoundMin, NParBound_Tot_5percent, CM_Pos_Relative_Error, CM_Vel_Relative_Error );
      MPI_Barrier( MPI_COMM_WORLD );
      ...
      if ( MPI_Rank == 0 )
      {
         ...
         if ( (MAX(CM_Pos_Relative_Error, CM_Vel_Relative_Error) <= 0.0001  ||  NIter >= NIterMax) && !((NParBound_Tot < NParBoundMin) && FirstTime_MassRest) )
         {
//          [Part IV] output CM information before exiting the loop
            ...
            fprintf( file_center, "%20.14e  %10ld  %14d  %10d  %14.7e  %14.7e  %14.7e  %14.7e  %14.7e  %14.7e  %14.7e %14.7e\n",
                     Time[0], Step, NParBound_Tot, NIter, Central_CM[0], Central_CM[1], Central_CM[2], Central_vel[0], Central_vel[1], Central_vel[2], CM_Pos_Relative_Error, CM_Vel_Relative_Error );
            fclose( file_center );
...
//       restore all particle masses and repeat the calculations as necessary
         if ((NParBound_Tot < NParBoundMin) && FirstTime_MassRest)
         {
            for (long p=0; p<ParN_ThisRank; p++)
            {
               if ( (PMass[p] > (real)0.0) &&  (PType[p] != PTYPE_TRACER) )
...
// [Part V] recompute potential on all grids
...
// Recompute the gravitational potential on all grids
   Recompute_Potential_BH_Buoyancy( EXT_POT_NONE );
...
//       compute and record the specific angular momentum for all particles
         amr->Par->Attribute[Idx_ParAngMom][p] = SQRT(SQR((Pos[1][p]-Central_CM[1])*(Vel[2][p]-Central_vel[2])
                                                 - (Pos[2][p]-Central_CM[2])*(Vel[1][p]-Central_vel[1])) +
                                                 SQR((Pos[2][p]-Central_CM[2])*(Vel[0][p]-Central_vel[0])
                                                 - (Pos[0][p]-Central_CM[0])*(Vel[2][p]-Central_vel[2])) +
                                                 SQR((Pos[0][p]-Central_CM[0])*(Vel[1][p]-Central_vel[1])
                                                 - (Pos[1][p]-Central_CM[1])*(Vel[0][p]-Central_vel[0])));
      }
   }
...
//    [Part VII] compute and record velocity anisotropy of the bound remnant
      ...
      int NParBound_Tot_Half    = NParBound_Tot/2;
...
//          record radial and tangential components of the velocity squared for ALL bound particles
            rx_p              = Pos[0][p]-Central_CM[0];
            ry_p              = Pos[1][p]-Central_CM[1];
            rz_p              = Pos[2][p]-Central_CM[2];
...
//             record radial and tangential components of the velocity squared for 50% most bound particles
               if ( amr->Par->Attribute[Idx_ParTag][p] <= NParBound_Tot_Half )
               {
      ...
      if ( MPI_Rank == 0 )
      {
         ...
         fprintf( file_beta, "%20.14e  %10ld  %14d  %14d  %14.7e  %14.7e\n",
                  Time[0], Step, NParBound_Tot, NParBound_Tot_Half, (double)beta_total, (double)beta_half );
         fclose( file_beta );
      } // if ( MPI_Rank == 0 )
   } // if ( OPT_beta_compute )


   ParEnergy_Calculation_BH_Buoyancy( false );
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Aux_Record_BH_Buoyancy
// Description :  Call Output_UserWorkBeforeOutput_BH_Buoyancy() at every global time-step
//
// Note        :  None
//
// Parameter   :  None
...
void Aux_Record_BH_Buoyancy()
{
   if ( Sorting_Step )   Output_UserWorkBeforeOutput_BH_Buoyancy();
   Sorting_Step = true;
} // FUNCTION : Aux_Record_BH_Buoyancy
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Init_TestProb_Hydro_BH_Buoyancy
...
// Return      :  None
//-------------------------------------------------------------------------------------------------------
void Init_TestProb_Hydro_BH_Buoyancy()
{
...
// set the problem-specific runtime parameters
   ...
   if ( OPT_Auto_AMR )    Flag_User_Ptr = Flag_BH_Buoyancy;

#  ifdef PARTICLE
   Par_Init_Attribute_User_Ptr = AddNewParticleAttribute_BH_Buoyancy;
#  endif
#  endif // #if ( MODEL == HYDRO )

   Output_UserWorkBeforeOutput_Ptr = Output_UserWorkBeforeOutput_BH_Buoyancy;
   Aux_Record_User_Ptr             = Aux_Record_BH_Buoyancy;
