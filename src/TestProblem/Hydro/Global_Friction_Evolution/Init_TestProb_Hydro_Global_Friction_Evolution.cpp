...
static int      NParBound_Sum;                           // total number of bound particles across all ranks
static int      Ter_NPar;                                // applicable when "OPT_Auto_Termination == 1"
...
static double   Perturber_Mass;                          // simulated (unmodified) individual perturber mass
static double   Central_Ini_R_vir;                       // central initial virial radius
static double   Central_Ini_vel_x;                       // central initial x-component velocity
static double   Central_Ini_vel_y;                       // central initial y-component velocity
static double   Central_Ini_vel_z;                       // central initial z-component velocity
static double   Central_Ini_vel_cir;                     // circular velocity at the central virial radius "= SQRT(G*M_vir/Central_Ini_R_vir)"
       double   Hosthalo_CM[3] = { __DBL_MAX__, __DBL_MAX__, __DBL_MAX__ };  // central center of mass coordinates
static double   Central_vel[3];                          // central center of mass velocity
...
static double   Hosthalo_M_vir;                          // host halo virial mass [code units]
static double   Hosthalo_R_scale;
static double   Hosthalo_fc;

#ifdef PARTICLE
void Init_ExtPot_Global_Friction_Evolution();
void AddNewParticleAttribute_Global_Friction_Evolution();
void Compute_Den_Max_On_Grid_Global_Friction_Evolution( double DenMaxOnGridPos[] );
void Output_UserWorkBeforeOutput_Global_Friction_Evolution();
void Aux_Record_Global_Friction_Evolution();
#endif
# ifdef GRAVITY
...
void Recompute_Potential_Global_Friction_Evolution( const int OptExtPot_Input );
void ParEnergy_Calculation_Global_Friction_Evolution( const bool OPT_Self_Total_Pot );
void ParEnergy_Sorting_Global_Friction_Evolution( const bool FirstTime_MassRest, long& NParBound_Sum_5percent,
...
// ********************************************************************************************************************************
   ...
   ReadPara->Add( "Perturber_Mass",            &Perturber_Mass,             1.0,           0.0,              NoMax_double      );
   ReadPara->Add( "Central_Ini_R_vir",         &Central_Ini_R_vir,          1.0,           0.0,              NoMax_double      );
   ReadPara->Add( "Central_Ini_vel_x",         &Central_Ini_vel_x,          0.0,           NoMin_double,     NoMax_double      );
   ReadPara->Add( "Central_Ini_vel_y",         &Central_Ini_vel_y,          0.0,           NoMin_double,     NoMax_double      );
   ReadPara->Add( "Central_Ini_vel_z",         &Central_Ini_vel_z,          0.0,           NoMin_double,     NoMax_double      );
   ReadPara->Add( "Central_Ini_vel_cir",       &Central_Ini_vel_cir,        0.0,           NoMin_double,     NoMax_double      );
...
// (1-2) set the default values
   Central_vel[0] = Central_Ini_vel_x;
   Central_vel[1] = Central_Ini_vel_y;
   Central_vel[2] = Central_Ini_vel_z;
...
// (4) make a note
   if ( MPI_Rank == 0 )
   {
      ...
      Aux_Message( stdout, "  individual central particle mass                           = %13.7e\n", Particle_Mass );
      Aux_Message( stdout, "  central initial virial radius                              = %13.7e\n", Central_Ini_R_vir );
      Aux_Message( stdout, "  central initial x velocity                                 = %13.7e\n", Central_Ini_vel_x );
      Aux_Message( stdout, "  central initial y velocity                                 = %13.7e\n", Central_Ini_vel_y );
      Aux_Message( stdout, "  central initial z velocity                                 = %13.7e\n", Central_Ini_vel_z );
      Aux_Message( stdout, "  central initial circular velocity at R_vir                 = %13.7e\n", Central_Ini_vel_cir );
...
//-------------------------------------------------------------------------------------------------------
// Function    :  AddNewParticleAttribute_Global_Friction_Evolution
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Recompute_Potential_Global_Friction_Evolution
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Compute_Den_Max_On_Grid_Global_Friction_Evolution
...
//-------------------------------------------------------------------------------------------------------
// Function    :  ParEnergy_Calculation_Global_Friction_Evolution
...
//       compute and record the particle kinetic energy
         ...
         {
            amr->Par->Attribute[Idx_ParEnergy][p] = 0.5*( SQR(Vel[0][p]-Central_vel[0]) + SQR(Vel[1][p]-Central_vel[1]) + SQR(Vel[2][p]-Central_vel[2]) );
//          mark both unbound particles and perturbers as (practically) massless
            if ( amr->Par->Attribute[Idx_ParTag][p] == (real)0.0 || amr->Par->Attribute[Idx_ParID][p] < (real)0.0 )    amr->Par->Mass[p] = 10.0*TINY_NUMBER;
         }
...
// 2. potential energy
   if (OPT_Self_Total_Pot)  Recompute_Potential_Global_Friction_Evolution( EXT_POT_NONE ); // temporarily exclude the background static gravitational potential and recompute potential on all grids
...
//-------------------------------------------------------------------------------------------------------
// Function    :  ParEnergy_Sorting_Global_Friction_Evolution
// Description :  Compute individual particle total energy
//
// Note        :  (1) self_potential + kinetic or (2) boosted + kinetic
//                [Part II] loop thorough all particles to (1) assign unbound particles negative mass and (2) determine the total energy of individual particles
...
// Return      :  None
//-------------------------------------------------------------------------------------------------------
void ParEnergy_Sorting_Global_Friction_Evolution( const int NParBoundMin, long& NParBound_Sum_5percent,
                        double& CM_Pos_Relative_Error, double& CM_Vel_Relative_Error )
{
// [Part III] relabel bound/unbound particles, and perform non-parallel sorting based on the particle total specific energy
   ...
   for (long p=0; p<ParN_ThisRank; p++)
   {
//    skip inactive particles (with negative mass), and tracers, and perturbers
      if ( PMass[p] > (real)0.0  &&  PType[p] != PTYPE_TRACER  &&  amr->Par->Attribute[Idx_ParID][p] > (real)0.0 )
      {
...
//    update the central CM position and velocity
      for (int v=0; v<3; v++)
      {
      ...
      CM_Pos_Relative_Error = SQRT(SQR(CM_Pos[0]-Hosthalo_CM[0])+SQR(CM_Pos[1]-Hosthalo_CM[1])+SQR(CM_Pos[2]-Hosthalo_CM[2]))/Central_Ini_R_vir;
      CM_Vel_Relative_Error = SQRT(SQR(CM_Vel[0]-Central_vel[0])+SQR(CM_Vel[1]-Central_vel[1])+SQR(CM_Vel[2]-Central_vel[2]))/Central_Ini_vel_cir;

      for (int v=0; v<3; v++)
      {
         Hosthalo_CM[v]  = CM_Pos[v];
         Central_vel[v] = CM_Vel[v];
      }
...
// broadcast
   ...
   MPI_Bcast( Hosthalo_CM, 3, MPI_DOUBLE, 0, MPI_COMM_WORLD );
   MPI_Bcast( Central_vel, 3, MPI_DOUBLE, 0, MPI_COMM_WORLD );
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Output_UserWorkBeforeOutput_Global_Friction_Evolution
...
//
//
// Note        :  1. The function iteratively determine the central CM by selecting a fixed percentage
...
//                   also temporarily disabled, after the central self-potential is recomputed accounting
//                   only for bound members of the central
...
//                4. Recompute the "Hosthalo_CM[3]" and "Central_vel[3]" from the 5% most bound particles,
...
void Output_UserWorkBeforeOutput_Global_Friction_Evolution()
{
   ...
   const char  filename_center_per[]  = "Record__Center__Perturber";
...
// adopt the on-grid maximum density position as the initial guess of the central CM
   if ( Step == (real)0 )  Compute_Den_Max_On_Grid_Global_Friction_Evolution( Hosthalo_CM );
...
//      Aux_Message( stdout, "      Before: Central_vel[0] = %14.7e, Central_vel[1] = %14.7e, Central_vel[2] = %14.7e\n", Central_vel[0], Central_vel[1], Central_vel[2]);
      ...
      for (long p=0; p<ParN_ThisRank; p++)
      {
//       skip inactive particles (with negative mass), tracers, and perturbers
         if ( (amr->Par->Attribute[Idx_ParTag][p] > (real)0.0) && (amr->Par->Attribute[Idx_ParTag][p] <= NParBound_Sum_5percent)
            &&  (PMass[p] > (real)0.0)  &&  (PType[p] != PTYPE_TRACER)  &&  (amr->Par->Attribute[Idx_ParID][p] > (real)0.0) )
         {
      ...
      if ( MPI_Rank == 0 )
      {
         ...
         for (int v=0; v<3; v++)
         {
            Hosthalo_CM[v]  = CM_Vel_Est_AllRank[v]/(double)ParVel_Sum_AllRank;
            Central_vel[v] = CM_Vel_Est_AllRank[v]/(double)ParVel_Sum_AllRank;
         }
         Aux_Message( stdout, "      After: Hosthalo_CM[0] = %14.7e,  Hosthalo_CM[1] = %14.7e,  Hosthalo_CM[2] = %14.7e\n", Hosthalo_CM[0], Hosthalo_CM[1], Hosthalo_CM[2]);
         Aux_Message( stdout, "      After: Central_vel[0] = %14.7e, Central_vel[1] = %14.7e, Central_vel[2] = %14.7e\n", Central_vel[0], Central_vel[1], Central_vel[2]);
      }
...
//       sort particle total energy entries
         ...
         for (int v=0; v<3; v++)
         {
            Hosthalo_CM[v]  = CM_Pos_Est[v]/(double)NParCMEst;
            Central_vel[v] = CM_Vel_Est[v]/(double)NParCMEst;
         }
         //Aux_Message( stdout, "      (MPI_Rank = %10d) After: Hosthalo_CM[0] = %14.7e,  Hosthalo_CM[1] = %14.7e,  Hosthalo_CM[2] = %14.7e\n", MPI_Rank, Hosthalo_CM[0], Hosthalo_CM[1], Hosthalo_CM[2]);
         //Aux_Message( stdout, "      (MPI_Rank = %10d) After: Central_vel[0] = %14.7e, Central_vel[1] = %14.7e, Central_vel[2] = %14.7e\n", MPI_Rank, Central_vel[0], Central_vel[1], Central_vel[2]);
...
//    [Part II] update individual particle energy entries
      ParEnergy_Calculation_Global_Friction_Evolution( true );

//    ============================================================================================================
//    [Part III] relabel bound/unbound particles, and perform non-parallel sorting based on the particle total specific energy
      NParBound_Sum = 0;                                // initialize/reset the value
      ParEnergy_Sorting_Global_Friction_Evolution( NParBoundMin, NParBound_Sum_5percent, CM_Pos_Relative_Error, CM_Vel_Relative_Error );
      MPI_Barrier( MPI_COMM_WORLD );
...
//          [Part IV] output CM information before exiting the loop
            if ( FirstTime )
            {
               ...
               if ( Aux_CheckFileExist(filename_center_per) )
                  Aux_Message( stderr, "WARNING : file \"%s\" already exists !!\n", filename_center_per );
               else
               {
                  FILE *file_center_per = fopen( filename_center_per, "w" );
                  fprintf( file_center_per, "#%19s  %10s  %14s  %14s  %14s  %14s  %14s  %14s\n",
                           "Time", "Step", "CM_x", "CM_y", "CM_z", "CM_Velx", "CM_Vely", "CM_Velz");
                  fclose( file_center_per );
               }
            ...
            fprintf( file_center, "%20.14e  %10ld  %14d  %10d  %14.7e  %14.7e  %14.7e  %14.7e  %14.7e  %14.7e  %14.7e %14.7e\n",
                     Time[0], Step, NParBound_Sum, NIter, Hosthalo_CM[0], Hosthalo_CM[1], Hosthalo_CM[2], Central_vel[0], Central_vel[1], Central_vel[2], CM_Pos_Relative_Error, CM_Vel_Relative_Error );
            fclose( file_center );
...
// Restore the particle mass of all unbound particles.
   for (long p=0; p<ParN_ThisRank; p++)
   {
      if ( (PMass[p] > (real)0.0 && PMass[p] < 0.5*Particle_Mass)  &&  PType[p] != PTYPE_TRACER  &&  (amr->Par->Attribute[Idx_ParID][p] > (real)0.0) )
         {
            amr->Par->Mass[p] = Particle_Mass;
         }
   }
...
// Restore the particle mass of perturbers and record their particle attributes
   for (long p=0; p<ParN_ThisRank; p++)
   {
      if ( PMass[p] > (real)0.0  &&  PType[p] != PTYPE_TRACER  &&  amr->Par->Attribute[Idx_ParID][p] < (real)0.0 )
      {
         amr->Par->Mass[p] = Perturber_Mass;

//       record the particle attributes of perturbers
         FILE *file_center_per = fopen( filename_center_per, "a" );
         fprintf( file_center_per, "%20.14e  %10ld  %14.7e  %14.7e  %14.7e  %14.7e  %14.7e  %14.7e\n",
                  Time[0], Step, Pos[0][p], Pos[1][p], Pos[2][p], Vel[0][p], Vel[1][p], Vel[2][p] );
         fclose( file_center_per );
      }
   }

// Recompute the gravitational potential on all grids
   //Recompute_Potential_Global_Friction_Evolution( EXT_POT_FUNC );
   Recompute_Potential_Global_Friction_Evolution( EXT_POT_NONE ); // recompute potential on all grids
...
//       compute and record the specific angular momentum for all particles
         amr->Par->Attribute[Idx_ParAngMom][p] = SQRT(SQR((Pos[1][p]-Hosthalo_CM[1])*(Vel[2][p]-Central_vel[2])
                                                 - (Pos[2][p]-Hosthalo_CM[2])*(Vel[1][p]-Central_vel[1])) +
                                                 SQR((Pos[2][p]-Hosthalo_CM[2])*(Vel[0][p]-Central_vel[0])
                                                 - (Pos[0][p]-Hosthalo_CM[0])*(Vel[2][p]-Central_vel[2])) +
                                                 SQR((Pos[0][p]-Hosthalo_CM[0])*(Vel[1][p]-Central_vel[1])
                                                 - (Pos[1][p]-Hosthalo_CM[1])*(Vel[0][p]-Central_vel[0])));
      }
   }
...
//    [Part VII] compute and record velocity anisotropy of the bound remnant
      ...
      for (long p=0; p<ParN_ThisRank; p++)
      {
//       skip inactive particles (with negative mass), tracer , and perturbers
         if ( PMass[p] > (real)0.0  &&  PType[p] != PTYPE_TRACER  &&  amr->Par->Attribute[Idx_ParID][p] > (real)0.0 )
         {
//          record radial and tangential components of the velocity squared for ALL bound particles
            rx_p              = Pos[0][p]-Hosthalo_CM[0];
            ry_p              = Pos[1][p]-Hosthalo_CM[1];
            rz_p              = Pos[2][p]-Hosthalo_CM[2];
            r_norm_p          = SQRT(SQR(rx_p) + SQR(ry_p) + SQR(rz_p));
            rx_p             /= r_norm_p;
            ry_p             /= r_norm_p;
            rz_p             /= r_norm_p;
            v_radial_sq_p     = SQR(rx_p*(Vel[0][p]-Central_vel[0]) + ry_p*(Vel[1][p]-Central_vel[1]) + rz_p*(Vel[2][p]-Central_vel[2]));
            v_tangential_sq_p = (SQR(Vel[0][p]-Central_vel[0]) + SQR(Vel[1][p]-Central_vel[1]) + SQR(Vel[2][p]-Central_vel[2])) - v_radial_sq_p;

//          record radial and tangential components of the velocity squared of ALL bound particles
            if ( amr->Par->Attribute[Idx_ParTag][p] > (real)0.0   &&  amr->Par->Attribute[Idx_ParID][p] > (real)0.0 )
            {
...
//             record radial and tangential components of the velocity squared for 50% most bound particles
               if ( amr->Par->Attribute[Idx_ParTag][p] <= NParBound_Sum_Half   &&  amr->Par->Attribute[Idx_ParID][p] > (real)0.0 )
               {
   ...
   ParEnergy_Calculation_Global_Friction_Evolution( false );
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Aux_Record_Global_Friction_Evolution
// Description :  Call Output_UserWorkBeforeOutput_Global_Friction_Evolution() at every global time-step
//
// Note        :  None
//
// Parameter   :  None
...
void Aux_Record_Global_Friction_Evolution()
{
   if ( Sorting_Step )   Output_UserWorkBeforeOutput_Global_Friction_Evolution();
   Sorting_Step = true;

// if the automatic simulation termination check is enabled
   if ( OPT_Auto_Termination )
   {
//    terminate the system if the central is disrupted
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Init_TestProb_Hydro_Global_Friction_Evolution
...
// Return      :  None
//-------------------------------------------------------------------------------------------------------
void Init_TestProb_Hydro_Global_Friction_Evolution()
{
...
// set the problem-specific runtime parameters
   SetParameter();
   Init_ExtPot_Ptr         = Init_ExtPot_Global_Friction_Evolution;
   Init_Function_User_Ptr  = SetGridIC;
   if ( OPT_Auto_AMR )    Flag_User_Ptr = Flag_Global_Friction_Evolution;

#  ifdef PARTICLE
   Par_Init_Attribute_User_Ptr = AddNewParticleAttribute_Global_Friction_Evolution;
#  endif
#  endif // #if ( MODEL == HYDRO )

   Output_UserWorkBeforeOutput_Ptr = Output_UserWorkBeforeOutput_Global_Friction_Evolution;
   Aux_Record_User_Ptr             = Aux_Record_Global_Friction_Evolution;
