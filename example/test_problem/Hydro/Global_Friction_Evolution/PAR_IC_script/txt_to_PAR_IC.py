...
Par_type               = 1.0
Perturber_Mass         = 1.0e-2                             # [NFW_Mvir]
Perturber_posx         = 10.0                               # relative to the simulation box center [NFW_Rscale]
Perturber_posy         = 0.0                                # relative to the simulation box center [NFW_Rscale]
Perturber_posz         = 0.0                                # relative to the simulation box center [NFW_Rscale]
Perturber_velx         = 0.0                                # in units of initial circular velocity at virial radius
Perturber_vely         = -1.0                               # in units of initial circular velocity at virial radius
Perturber_velz         = 0.0                                # in units of initial circular velocity at virial radius
...
#append the perturber info
Output_Mass           = np.append(Output_Mass, Perturber_Mass)
Output_Pos_x          = np.append(Output_Pos_x, Perturber_posx + Center_of_Mass_X)
Output_Pos_y          = np.append(Output_Pos_y, Perturber_posy + Center_of_Mass_Y)
Output_Pos_z          = np.append(Output_Pos_z, Perturber_posz + Center_of_Mass_Z)
Output_Vel_x          = np.append(Output_Vel_x, Perturber_velx*NFW_Vvir)
Output_Vel_y          = np.append(Output_Vel_y, Perturber_vely*NFW_Vvir)
Output_Vel_z          = np.append(Output_Vel_z, Perturber_velz*NFW_Vvir)
Output_ParType        = np.append(Output_ParType, Par_type)
Output_ParTag         = np.append(Output_ParTag, 1.0)
Output_ParEnergy      = np.append(Output_ParEnergy, 0.0)
Output_ParAngMom      = np.append(Output_ParAngMom, 0.0)
Output_ParEnergyBac   = np.append(Output_ParEnergyBac, 0.0)
NPar_AllRank          = NPar_AllRank+1
Output_ParID          = np.append(Output_ParID, -1)
...
with open('PAR_IC__Input__TestProb', 'w') as writer:
   ...
   writer.write("Perturber_Mass          %.14e         # [NFW_Mvir]\n"%Perturber_Mass)
   writer.write("Perturber_posx          %.14e         # relative to the simulation box center [NFW_Rscale]\n"%Perturber_posx)
   writer.write("Perturber_posy          %.14e         # relative to the simulation box center [NFW_Rscale]\n"%Perturber_posy)
   writer.write("Perturber_posz          %.14e         # relative to the simulation box center [NFW_Rscale]\n"%Perturber_posz)
   writer.write("Perturber_velx          %.14e         # in units of initial circular velocity at virial radius\n"%Perturber_velx)
   writer.write("Perturber_vely          %.14e         # in units of initial circular velocity at virial radius\n"%Perturber_vely)
   writer.write("Perturber_velz          %.14e         # in units of initial circular velocity at virial radius\n"%Perturber_velz)
