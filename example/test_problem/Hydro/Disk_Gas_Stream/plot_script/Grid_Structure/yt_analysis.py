...
for idx in range(idx_start,idx_end+1,didx):
   ...
   #print(ds.parameters)
   '''
   BoxSize     = ds.parameters["boxlen"]

   plot = yt.LinePlot(ds, [("gas", "velocity_x"),("gas", "velocity_y"),("gas", "velocity_z")], [0.0, 0.5, 0.5], [1.0, 0.5, 0.5], 1024)
   plot.annotate_legend([("gas", "velocity_x"),("gas", "velocity_y"),("gas", "velocity_z")])
   plot.set_x_unit("kpc")
   plot.set_unit(("gas", "velocity_x"), "km/s")
   plot.annotate_title(("gas", "velocity_x"), "Gas disk velocity along x-axis (midplane)")
   plot.set_ylabel("Velocity (km/s)")
   #plot.ylim(('gas', 'velocity_x'), (Gas_vel_lower, "km/s"), (Gas_vel_upper, "km/s"))
   plot.save()

   plot = yt.LinePlot(ds, ("gas", "density"), [0.0, 0.5, 0.5], [1.0, 0.5, 0.5], 1024)
   plot.annotate_legend(("gas", "density"))
   plot.set_x_unit("kpc")
   plot.set_unit(("gas", "density"), "Msun/kpc**3")
   plot.annotate_title(("gas", "density"), "Gas disk density along x-axis (midplane)")
   #plot.set_ylim(('gas', 'density'), (Gas_den_lower, "Msun/kpc**3"), (Gas_den_upper, "Msun/kpc**3"))
   plot.save()

   plot = yt.LinePlot(ds, ("gas", "temperature"), [0.0, 0.5, 0.5], [1.0, 0.5, 0.5], 1024)
   plot.annotate_legend(("gas", "temperature"))
   plot.set_x_unit("kpc")
   plot.set_unit(("gas", "temperature"), "Kelvin")
   plot.annotate_title(("gas", "temperature"), "Gas disk temperature along x-axis (midplane)")
   #plot.set_ylim(('gas', 'temperature'), (Gas_temp_lower, "Kelvin"), (Gas_temp_upper, "Kelvin"))
   plot.save()

   plot = yt.LinePlot(ds, ("gas", "pressure"), [0.0, 0.5, 0.5], [1.0, 0.5, 0.5], 1024)
   plot.annotate_legend(("gas", "pressure"))
   plot.set_x_unit("kpc")
   plot.set_unit(("gas", "pressure"), "erg/cm**3")
   plot.annotate_title(("gas", "pressure"), "Gas disk pressure along x-axis (midplane)")
   #plot.set_ylim(('gas', 'temperature'), (Gas_pre_lower, "erg/cm**3"), (Gas_pre_upper, "erg/cm**3"))
   plot.save()
   '''
   '''
   plot = yt.LinePlot(ds, [("gas", "velocity_x"),("gas", "velocity_y"),("gas", "velocity_z")], [0.5, 0.5, 0.0], [0.5, 0.5, 1.0], 1024)
   plot.annotate_legend([("gas", "velocity_x"),("gas", "velocity_y"),("gas", "velocity_z")])
   plot.set_x_unit("kpc")
   plot.set_unit(("gas", "velocity_x"), "km/s")
   plot.annotate_title(("gas", "velocity_x"), "Gas disk velocity along z-axis (Zenith)")
   plot.set_ylabel("Velocity (km/s)")
   plot.save()

   plot = yt.LinePlot(ds, ("gas", "density"), [0.5, 0.5, 0.0], [0.5, 0.5, 1.0], 1024)
   plot.annotate_legend(("gas", "density"))
   plot.set_x_unit("kpc")
   plot.set_unit(("gas", "density"), "Msun/kpc**3")
   plot.annotate_title(("gas", "density"), "Gas disk density along z-axis (Zenith)")
   plot.save()

   plot = yt.LinePlot(ds, ("gas", "temperature"), [0.5, 0.5, 0.0], [0.5, 0.5, 1.0], 1024)
   plot.annotate_legend(("gas", "temperature"))
   plot.set_x_unit("kpc")
   plot.set_unit(("gas", "temperature"), "Kelvin")
   plot.annotate_title(("gas", "temperature"), "Gas disk temperature along z-axis (Zenith)")
   plot.save()

   plot = yt.LinePlot(ds, ("gas", "pressure"), [0.5, 0.5, 0.0], [0.5, 0.5, 1.0], 1024)
   plot.annotate_legend(("gas", "pressure"))
   plot.set_x_unit("kpc")
   plot.set_unit(("gas", "pressure"), "erg/cm**3")
   plot.annotate_title(("gas", "pressure"), "Gas disk pressure along z-axis (Zenith)")
   plot.save()
   '''
   '''
