...
# load the total number of particle entries expected
NPar                  = int(float(Input_Parameter_Readline("../Input__Parameter", "PAR_NPAR     ", 1)))
...
for idx in range(idx_start,idx_end+1,didx):
    ...
    if (Template == 1):
        ...
        with open(Output_filename, 'w') as master_output:
            # output header
            master_output.write("pmass [Msun]             posx [pc]                posy [pc]                posz [pc]                velx [pc/Myr]            vely [pc/Myr]            velz [pc/Myr]            ptype                    ptag                     peng                     pangmom                  pEnergyBac               pID                      \n")
            # output simulation data
            for output_index in range(len(posx_list)):
                    master_output.write(format("%.14e"%pmass_list[output_index],"<25"))
                    master_output.write(format("%.14e"%posx_list[output_index],"<25"))
                    ...
                    master_output.write(format("%.14e"%pEnergyBac_list[output_index],"<25"))
                    master_output.write(format("%.14e"%pID_list[output_index],"<25"))
                    master_output.write("\n")